...
- **Live Degoog verification**: an authenticated-disabled tunneled instance returned 23 native results from two healthy engines. CLI, config-file, and environment provider selection all returned normalized results; Markdown and JSON limits were respected; `--format raw` preserved the native `engineTimings` response; `--quiet` produced zero stderr bytes; and supplying an optional bearer token remained accepted without exposing it in output.
- **Current state**: complete and committed in `c45798c`. Provider-neutral `SearchClient`, native SearXNG/Degoog adapters, typed provider config, generic CLI routing, raw preservation, bearer authentication, redacted effective config output, generalized diagnostics, documentation, and focused tests are implemented.
